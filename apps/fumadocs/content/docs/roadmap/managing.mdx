...
A note on status values: the board columns are `backlog`, `next`, `progress`, and `done`, but the API takes the backend names. `under_review` and `considering` land in Under Review, `planned` and `in_progress` map across to the active columns, and `shipped` or `closed` land in Done.
...
<Screenshot caption="A roadmap item open in detail, with stats and source evidence." ratio="16 / 10" />
