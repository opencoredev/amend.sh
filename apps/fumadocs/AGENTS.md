...
| --- | --- |
| Feedback / posts board, post composer, voting, comments, reactions | `customer-surfaces.mdx`, `api-reference.mdx` |
| Public portal (identity, submission, feeds) | `customer-surfaces.mdx`, `integration.mdx` |
| Embed panel — `createAmendPanel`, `packages/sdk` `embed.ts` | `customer-surfaces.mdx` |
| Roadmap workspace, statuses (`backlog`/`next`/`progress`/`done`) | `source-trace.mdx`, `index.mdx` (loop table) |
| Changelog editor + AI changelog drafting | `automation.mdx`, `api-reference.mdx` |
...
- **Changelog editor UI** — the interactive editor/review/publish flow (only AI drafting via API is documented)
