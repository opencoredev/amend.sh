...
import { ensureBaseRecords, ensureGitHubConnection } from "../demo/amendSeed";
import {
  createSourceLink,
...
export async function trustedIngestSourceEventHandler(
  ...
  // Shipped-work side effects (auto changelog draft, roadmap/review updates,
  // notifications) require HMAC-verified GitHub provenance — a bearer-token
  // signal claiming kind "release" must never fabricate a ship. The signal-bus
  // endpoint also rejects these kinds up front; this is defense in depth.
  if (provider === "github" && isShippedSourceEvent(args)) {
