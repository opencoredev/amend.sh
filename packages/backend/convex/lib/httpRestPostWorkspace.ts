...
export async function handleWorkspaceRestPost(input: RestPostInput) {
  ...
  if (resource === "deliveries") {
    if (body.action === "send") {
      ...
      const limit = numberValue(body.limit) ?? 25;
      const channel = deliveryChannel(body.channel);
      // Real sends claim rows transactionally (see deliveryScheduler.ts) so a
      // manual drain cannot double-send against a concurrent cron tick.
      const deliveries = dryRun
        ? await ctx.runQuery(internal.amend.getDeliveryOutboxForApi, { workspaceSlug })
        : await ctx.runMutation(internal.deliveryScheduler.claimQueuedDeliveries, {
            ...(channel ? { channel } : {}),
            limit,
            workspaceSlug,
          });
