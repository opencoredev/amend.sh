...
import { escapeHtml } from "../lib/httpRuntimeScalars";
...
export async function sendQueuedDeliveries(
  ...
  const queued = deliveries
    // "sending" rows were claimed for this drain by claimQueuedDeliveries.
    .filter((delivery) => delivery.status === "queued" || delivery.status === "sending")
