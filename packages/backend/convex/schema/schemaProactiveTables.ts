...
import { sourceProvider } from "./schemaShared";

export const proactiveTables = {
  // 2026-07-01: Signal-bus routing table. Maps a provider-scoped routing key
  // (Discord guildId, Slack team_id, inbound email alias, "owner/repo", …) to
  // the workspace that claimed it. This replaces per-provider env defaults
  // (DISCORD_DEFAULT_WORKSPACE_SLUG) for multi-tenant ingest: unrouted signals
  // are rejected unless the dev-only fallback flag is set. A (provider,
  // routingKey) pair belongs to at most one workspace; release keeps the row
  // (state "disabled") so history survives and another workspace can claim it.
  channelRoutes: defineTable({
    workspaceId: v.id("workspaces"),
    provider: sourceProvider,
    routingKey: v.string(),
    state: v.union(v.literal("active"), v.literal("disabled")),
    config: v.object({
      // Whether the source worker should acknowledge captured signals (e.g. the
      // Discord 👀 reaction). Absent means enabled — only an explicit `false`
      // turns acks off.
      ackReaction: v.optional(v.boolean()),
      // Optional server-side channel allowlist (defense in depth alongside the
      // gateway worker's own allowlists). Absent means all channels.
      listenChannels: v.optional(v.array(v.string())),
    }),
    createdAt: v.number(),
    updatedAt: v.number(),
  })
    .index("by_provider_and_routingKey", ["provider", "routingKey"])
  ...
  memoryRules: defineTable({
    ...
    .index("by_workspace_and_sourceNeedId", ["workspaceId", "sourceNeedId"]),
