...
/**
 * Single source of truth for the proactive source-channel union. The validator
 * below and every hand-rolled check (e.g. `isSourceChannel` in pipeline.ts)
 * derive from this list so new channels only get added here.
 */
export const PROACTIVE_SOURCE_CHANNELS = [
  "discord",
  "support",
  "github",
  "embed",
...
export type ProactiveSourceChannel = (typeof PROACTIVE_SOURCE_CHANNELS)[number];

export const proactiveSourceChannelValue = v.union(
  ...PROACTIVE_SOURCE_CHANNELS.map((channel) => v.literal(channel)),
);
