...
import { requireDashboardUser, requireDashboardWorkspace } from "../workspace/amendWorkspace";
import type { ProactiveSourceChannel } from "./proactiveValidators";
...
export type ProactiveGhost = {
  ...
  proof: {
    people: number;
    payingPeople: number;
    sources: { channel: ProactiveSourceChannel; count: number }[];
    strength: "thin" | "building" | "strong";
    growthPerWeek: number;
  };
  sampleQuotes: { text: string; author: string; channel: ProactiveSourceChannel; url: string }[];
  firstSeen: number;
  lastSeen: number;
};
...
export type ProactiveEvidence = {
  id: string;
  sourceChannel: ProactiveSourceChannel;
  author: string;
  text: string;
  url: string;
  ...
  return {
    ...
    evidence: evidenceDocs
      ...
      .map((evidence) => ({
        ...
        ...(evidence.handle ? { authorHandle: evidence.handle } : {}),
        ...(evidence.authorAvatarUrl ? { authorAvatarUrl: evidence.authorAvatarUrl } : {}),
        text: evidence.text,
        url: evidence.url,
