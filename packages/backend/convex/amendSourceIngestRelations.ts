...
export async function updateRelatedFeedback(
  ...
  for (const feedback of input.relatedFeedback) {
    ...
    const linkedChangelogEntryIds = feedback.linkedChangelogEntryIds.includes(
      input.changelogEntryId,
    )
...
export async function updateRelatedRoadmap(
  ...
  for (const roadmapItem of input.relatedRoadmap) {
    ...
    const changelogEntryIds = roadmapItem.changelogEntryIds.includes(input.changelogEntryId)
    ...
    await upsertAutomationDecision(ctx, {
      ...
      summary: `Updated roadmap status from shipped GitHub source evidence: ${input.args.title}`,
      sourceLinks: [input.sourceLink],
      updatedAt: input.now,
    });
  }
